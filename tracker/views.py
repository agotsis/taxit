...
def generate_calendar_month(year, month, start_date=None, end_date=None):
    ...
    days = Day.objects.filter(date__range=[month_start, month_end]).prefetch_related(
        "states", "office"
    )
    ...
    for week in cal:
        ...
        for day_num in week:
            if day_num == 0:
                ...
                week_data.append({"date": day_date, "day": day_obj, "in_range": in_range})
...
def year_view(request, year=None, month=None):
    ...
    days = Day.objects.filter(date__range=[start_date, end_date]).prefetch_related(
        "states", "office"
    )
    ...
    for state in states:
        ...
        if count > 0:
            state_counts[state] = {
                ...
                "percentage": (
                    (count / state.day_threshold * 100) if state.day_threshold > 0 else 0
                ),
            }
    ...
    for state in states:
        ...
        if count > 0:
            state_counts[state] = {
                ...
                "ratio": (
                    (count / total_workdays_logged * 100) if total_workdays_logged > 0 else 0
                ),
            }
